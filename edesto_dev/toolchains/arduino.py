...
def _build_boards() -> dict[str, Board]:
    """Build the dict of all 17 Arduino board definitions."""
    ...
    _reg(Board(
        ...
        baud_rate=115200,
        openocd_target="stm32f4x",
        capabilities=["digital_io", "analog_input", "pwm", "i2c", "spi", "uart", "dac", "usb_otg", "accelerometer"],
        pins={
            "onboard_led_green": 12,
            "onboard_led_orange": 13,
            "onboard_led_red": 14,
            "onboard_led_blue": 15,
            "user_button": 0,
            "i2c_sda": 9,
        ...
        pin_notes=[
            ...
            "B1 user button on PA0",
            "LIS3DSH accelerometer on SPI1 (PA5-PA7, PE3 CS)",
            "CS43L22 audio DAC on I2C1 + I2S3",
            "USB OTG micro-AB connector (PA11/PA12)",
            "ADC: 3x 12-bit ADCs, up to 24 channels",
            "DAC: 2 channels (PA4, PA5)",
            "STM32F407VGT6: 1MB flash, 192KB RAM, 168MHz",
        ],
        pitfalls=[
            "Upload is via ST-Link V2 (onboard), not USB serial. Install ST-Link drivers.",
            "Serial output requires an external USB-UART adapter or ST-Link VCP (not available on all revisions).",
            "Arduino pin mapping differs from STM32 native pin names (PA0, PB3, etc.).",
            "PA5 is shared between SPI1 SCK and DAC1. Cannot use both simultaneously.",
            "USB OTG and USB ST-Link are separate connectors — use the correct one.",
            "Library compatibility varies — not all Arduino libraries work with STM32.",
            "delay() blocks. Use millis() or HAL_GetTick() for non-blocking timing.",
        ],
        includes={},
    ...
    _reg(Board(
        slug="stm32h7-nucleo",
        name="STM32H7 Nucleo-144",
        fqbn="STMicroelectronics:stm32:Nucleo_144",
        core="STMicroelectronics:stm32",
        core_url="https://github.com/stm32duino/BoardManagerFiles/raw/main/package_stmicroelectronics_index.json",
        baud_rate=115200,
        openocd_target="stm32h7x",
        capabilities=["digital_io", "analog_input", "pwm", "i2c", "spi", "uart", "dac", "ethernet", "usb_otg", "dual_core"],
        pins={
            "onboard_led_green": 0,
            "onboard_led_blue": 7,
            "onboard_led_red": 14,
        ...
        pin_notes=[
            "3 user LEDs: PB0 (green), PB7 (blue), PB14 (red)",
            "B1 user button on PC13",
            "Ethernet via RJ45 connector (RMII)",
            "USB OTG FS connector",
            "Arduino-compatible and ST morpho headers",
            "ADC: 3x 16-bit ADCs",
            "DAC: 2x 12-bit channels",
            "STM32H743ZIT6: 2MB flash, 1MB RAM, 480MHz",
        ],
        pitfalls=[
            "Dual-core (Cortex-M7 + M4) requires careful memory domain management.",
            "D-Cache and I-Cache must be managed for DMA operations — invalidate/clean cache around DMA transfers.",
            "MPU configuration is recommended for memory protection between cores.",
            "480MHz generates significant heat — consider thermal management for sustained loads.",
            "Upload is via ST-Link V3, not USB serial. Install ST-Link drivers.",
            "Multiple clock domains — verify peripheral clock configuration carefully.",
            "Arduino pin mapping differs from STM32 native pin names.",
            "Library compatibility varies — not all Arduino libraries work with STM32H7.",
            "delay() blocks. Use millis() or HAL_GetTick() for non-blocking timing.",
        ],
        includes={},
    ))
    ...
    _reg(Board(
        slug="stm32l4-nucleo",
        name="STM32L4 Nucleo-64",
        fqbn="STMicroelectronics:stm32:Nucleo_64",
        core="STMicroelectronics:stm32",
        core_url="https://github.com/stm32duino/BoardManagerFiles/raw/main/package_stmicroelectronics_index.json",
        baud_rate=115200,
        openocd_target="stm32l4x",
        capabilities=["digital_io", "analog_input", "pwm", "i2c", "spi", "uart", "dac", "low_power"],
        pins={
            "onboard_led": 5,
            "user_button": 13,
            "i2c_sda": 14,
        ...
        pin_notes=[
            ...
            "B1 user button on PC13",
            "Arduino-compatible headers: D0-D15, A0-A5",
            "ADC: 12-bit resolution, internal voltage reference",
            "DAC: 2 channels (PA4, PA5)",
            "I2C: D14 (SDA), D15 (SCL)",
            "SPI: D11 (MOSI), D12 (MISO), D13 (SCK)",
            "UART via ST-Link Virtual COM Port (VCP)",
            "STM32L476RGT6: 1MB flash, 128KB RAM, 80MHz",
        ],
        pitfalls=[
            "Ultra-low-power chip — default clock is MSI at 4MHz, not full 80MHz. Configure clocks explicitly.",
            "Upload is via ST-Link, not USB serial. Install ST-Link drivers.",
            "Serial output is via ST-Link VCP, not native USB serial.",
            "Arduino pin mapping differs from STM32 native pin names (PA0, PB3, etc.).",
            "Low-power modes (Stop, Standby) require careful peripheral re-initialization on wake.",
            "Library compatibility varies — not all Arduino libraries work with STM32.",
            "delay() blocks. Use millis() or HAL_GetTick() for non-blocking timing.",
        ],
        includes={},
    ...
    _reg(Board(
        ...
        core="adafruit:nrf52",
        core_url="https://adafruit.github.io/arduino-board-index/package_adafruit_index.json",
        baud_rate=115200,
        openocd_target="nrf52",
        capabilities=["ble", "usb_native", "nfc", "qspi", "digital_io", "analog_input", "pwm", "i2c", "spi", "uart"],
        pins={
            "onboard_led_red": 3,
            "onboard_neopixel": 16,
            "user_switch": 7,
        ...
        pin_notes=[
            "Red LED on P0.03, NeoPixel on P0.16",
            "User switch on P0.07",
            "BLE 5.0 with Long Range (Coded PHY) support",
            "Native USB (nRF52840 has USB 2.0 controller)",
            "NFC antenna pins (P0.09/P0.10) — configure as NFC or GPIO",
            "QSPI flash: 2MB external (GD25Q16C)",
            "ADC: 6 channels, 14-bit resolution (12-bit effective)",
            "Battery voltage on P0.29 via voltage divider (multiply by 2)",
            "I2C: SDA=P0.25, SCL=P0.26",
            "SPI: MOSI=P0.24, MISO=P0.23, SCK=P0.14",
        ],
        pitfalls=[
            "No WiFi — BLE only. Use a separate WiFi module if internet connectivity is needed.",
            "NFC pins (P0.09/P0.10) default to NFC mode. Must configure as GPIO in firmware if needed for I/O.",
            "Adafruit bootloader uses UF2 — double-tap reset to enter bootloader (shows as USB drive).",
            "SoftDevice (BLE stack) consumes ~28KB RAM. Plan memory usage accordingly.",
            "BLE and QSPI flash share radio/SPI resources — manage timing carefully.",
            "Battery ADC (P0.29) reads through a voltage divider — multiply raw value by 2.",
            "delay() blocks. Use millis() for non-blocking timing.",
            "Arduino libraries for nRF52 are not always compatible with AVR/ESP32 libraries.",
        ],
        includes={
            "ble": "#include <bluefruit.h>",
        },
    ))
    ...
    _reg(Board(
        ...
        core_url="",
        baud_rate=115200,
        openocd_target="nrf5340",
        capabilities=["ble", "usb_native", "nfc", "dual_core", "trustzone", "digital_io", "analog_input", "pwm", "i2c", "spi", "uart"],
        pins={
            "onboard_led1": 28,
            "onboard_led2": 29,
            "onboard_led3": 30,
        ...
        pin_notes=[
            "4 user LEDs: P0.28-P0.31",
            "4 user buttons: P0.23, P0.24, P0.08, P0.09",
            "BLE 5.3 with Direction Finding",
            "Dual-core: Application (Cortex-M33, 128MHz) + Network (Cortex-M33, 64MHz)",
            "ARM TrustZone for secure/non-secure partitioning",
            "Native USB 2.0 on application core",
            "NFC-A tag support",
            "External memory interface (QSPI)",
            "ADC: 12-bit, 8 channels (application core only)",
        ],
        pitfalls=[
            "No Arduino core available — use Zephyr RTOS or nRF Connect SDK instead.",
            "Dual-core requires separate firmware images for application and network cores.",
            "BLE stack runs on the network core — application core communicates via IPC.",
            "TrustZone must be configured before non-secure code can run.",
            "Upload via J-Link (onboard) or external debug probe.",
            "Zephyr board name: nrf5340dk_nrf5340_cpuapp (app core) / nrf5340dk_nrf5340_cpunet (net core).",
            "Complex build system — west (Zephyr meta-tool) is strongly recommended.",
        ],
        includes={},
    ))
